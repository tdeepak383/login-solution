...
function Attherate() {
   const [contacts, setContacts] = useState([]);
    ...
    useEffect(() => {
      const fetchData = async () => {
        const response = await fetch(`${import.meta.env.VITE_VERCEL_URL}/api/attherate`);
    ...
    if (window.confirm('Are you sure you want to delete this contact?')) {
      ...
      fetch(`${import.meta.env.VITE_VERCEL_URL}/api/attherate/${id}`, {
        method: 'DELETE'
      })
  ...
  return (
    <div className='p-10 bg-gray-100 lg:w-full w-[1200px]'>
        <h2 className='text-2xl mb-5 font-bold'>Attherate Solutions Contacts List</h2>
        <div className='mt-10'>
        <div className='bg-white p-5 rounded-xl shadow-xl'>
          <div className="flex justify-between">
           ...
           <div>
            <ExportDropdown baseUrl={`${import.meta.env.VITE_VERCEL_URL}/api/attherate`} />
           </div>
          </div>
          <div className='mt-5'>
              ...
              <tbody>
                  {
                    ...
                    <tr key={item.id} className="border-b">
                      ...
                      <td className="py-2 align-middle text-left"><p className="text-sm">{item.full_name}</p></td>
                      <td><p className="text-sm line-clamp-1">{item.email}</p></td>
                      <td><p className="text-sm line-clamp-1">{item.phone}</p></td>
                      <td><p className="text-sm line-clamp-1">{item.subject}</p></td>
                      <td><p className="text-sm ">{item.message}</p></td>
